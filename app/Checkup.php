...
class Checkup extends Model
{
    ...
    public function getSummary( $checkupId ) {
        return DB::select(" SELECT 1 ORDEM, CK.ID, CK.TITULO, TP.DS_ATENDIMENTO, E.DS_ESPECIALIDADE ESPECIALIDADE, COUNT(1) QTY
                              FROM CHECKUPS CK
                            ...
                            UNION
                            SELECT 2 ORDEM, CK.ID, CK.TITULO, TP.DS_ATENDIMENTO, GP.DS_GRUPO ESPECIALIDADE, COUNT(1) QTY
                              FROM CHECKUPS CK
                              JOIN ITEM_CHECKUPS ICK ON (CK.ID = ICK.CHECKUP_ID)
                              JOIN ATENDIMENTOS AT ON (ICK.ATENDIMENTO_ID = AT.ID)
